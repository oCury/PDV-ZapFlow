...
vi.mock("@/lib/tenant/context", () => ({ getTenantId: vi.fn(() => "tenant_1") }));
vi.mock("@/lib/mercadopago/connection", () => ({
  resolveMpAccessToken: vi.fn(async () => "TEST_TOKEN"),
}));
vi.mock("@/lib/auth", () => ({
  requireTenant: vi.fn(async () => ({ tenantId: "tenant_1", userId: "u1", role: "ADMIN" })),
}));
