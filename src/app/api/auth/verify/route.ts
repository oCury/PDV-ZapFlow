...
export async function POST(req: NextRequest) {
  ...
  if (!token || typeof token !== "string") return NextResponse.json({ error: "Token ausente." }, { status: 400 });
  ...
  const res = await createTenantWithAdmin({
    ...
    passwordHash: pending.password_hash, plan: pending.plan, trialEndsAt, adminName: pending.name,
  });
  await basePrisma.pendingSignup.delete({ where: { id: pending.id } });

  await setSessionCookie({ userId: res.userId, role: "ADMIN", name: pending.name, tenantId: res.tenantId, trialEndsAt: trialEndsAt.toISOString() });
